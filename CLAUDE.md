...
- **도구 페이지 15개**: 각 페이지별 openGraph + twitter card + JSON-LD 구조화 데이터 포함
...
- `/tools/json-to-csv-converter` — JSON ↔ CSV Converter
...
- **JSON ↔ CSV Converter** — 양방향 변환, 중첩 JSON flatten (dot notation), 커스텀 구분자 4종, 파일 업로드/다운로드, 타입 자동추론, RFC 4180 quoting (외부 패키지 없음)

개발 예정 (우선순위 순):
1. **String Case Converter** — camelCase ↔ snake_case ↔ kebab-case ↔ PascalCase
2. **Number Base Converter** — 2/8/10/16진수 변환
3. **Password Generator** — 보안 랜덤 비밀번호 생성 (Web Crypto API)
...
- `components/landing/tools-landing.tsx` — **Server**, 15개 툴 hover-effect 그리드 + 3개 coming soon 섹션 (lucide-react 아이콘)
- `components/layout/navbar.tsx` — Fixed top navbar (server)
- `components/layout/footer.tsx` — **Client**, site-wide footer
- `components/layout/tool-nav-sidebar.tsx` — Tool navigation sidebar (server), 15개 도구 링크
...
- **Google AdSense**: 설치 완료 (`ca-pub-3842295775842337`). `components/ads/ad-unit.tsx` 컴포넌트로 각 도구 페이지에 삽입됨
